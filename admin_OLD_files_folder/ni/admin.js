// ============================================
// KSSS Tournament Control Center - Admin Panel
// Version: 2.2.4 - Security Hardened
// ============================================
(function () {
    'use strict';
    ...
    const AdminSecurity = (() => {
        let role = null;
        let isInitializing = false; // Rule 21: Initialization guard
        ...
        async function verifyRole(storedObj) {
            ...
            // Rule 9: Role Whitelisting - Only accept known role constants
            ...
            console.warn("🔒 Security Alert: Admin Role Tampered or Environmental Integrity Failure. Clearing session.");
        ...
        async function verifySession() {
            isInitializing = true; // Rule 21: Block all hooks during initialization

            // Rule 8: Dual Check - Token must exist
            const token = sessionStorage.getItem("githubToken");
            if (!token || token === "null" || token === "undefined") {
                console.warn("🔒 Session invalid: No token");
                await setRole(null); // Explicit cleanup
                isInitializing = false;
                showLoginModal();
                return;
            }

            // Rule 8: Dual Check - Role must exist and be valid
            const storedRole = sessionStorage.getItem("secureAdminRole");
            if (!storedRole) {
                console.warn("🔒 Session invalid: No role");
                await setRole(null); // Explicit cleanup
                isInitializing = false;
                showLoginModal();
                return;
            ...
            try {
                ...
                if (!roleVerified) {
                    // verifyRole logged the specific security alert
                    await setRole(null); // Explicit cleanup
                    isInitializing = false;
                    showLoginModal();
                    return;
            ...
            // Both token and role verified - proceed with initialization
            showRoleBadge();
            await loadMatches(); // Rule 15: Wait for load to complete before releasing lock
            freezeCriticalFunctions();

            isInitializing = false; // Initialization complete - allow UI interactions
        }
        ...
        async function validateSession() {
            ...
            // 2. Check if role exists and matches internal state
            const storedRole = sessionStorage.getItem("secureAdminRole");
            ...
            try {
                ...
                // Tamper detected if signature invalid OR if it changed role from internal state
        ...
        // AdminSecurity is now only accessible within this IIFE closure
        // No direct window export - only through KSSS_UI_HOOKS
        return instance;
    })();
    ...
    const CONFIG = {
        ...
        version: "2.2.4", // Version tracking for cache busting
    ...
    async function verifyIntegrity() {
        ...
        // 1. Check if Secure Module is intact (Frozen in memory)
        ...
        // 2. Deep Session Check (Signature in storage)
        ...
        if (!isSessionValid) {
            ...
            // Use non-blocking alert/logout to allow script to finish recording
            setTimeout(() => {
                alert("🔒 Security Alert: Session integrity compromised. Logging out.");
                AdminSecurity.logout(); // Trigger immediate safety logout
            }, 100);
    ...
    async function fetchWithRetry(url, options = {}, maxRetries = CONSTANTS.MAX_RETRIES) {
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            try {
                // PATCH: Support Fine-Grained PATs by switching 'token' to 'Bearer'
                if (options && options.headers && options.headers.Authorization) {
                    const auth = options.headers.Authorization;
                    if (auth.startsWith("token ")) {
                        const tokenVal = auth.substring(6).trim();
                        if (tokenVal.startsWith("gith") || tokenVal.length > 50) {
                            if (CONFIG?.debug) console.log("🔐 Auth Patch: Switching 'token' to 'Bearer'");
    ...
    async function loadMatches(forceRefresh = false) {
        ...
        try {
            ...
            // Show user-friendly error alert after overlay is hidden
            setTimeout(() => alert(`Failed to load matches:\n\n${e.message}\n\nPlease check:\n• Your GitHub token is valid\n• You have internet connection\n• The repository exists`), 100);
        } finally {
            document.getElementById("loading-overlay").classList.add("hidden");
            document.getElementById("editor-section").classList.remove("hidden");
            if (loadBtn) setButtonLoading(loadBtn, false);
    ...
    function addRoundManagementControls(container, round, rIdx) {
        ...
        const header = createEl("div", "", "📋 Round Management (v2.2.3)", "font-weight: bold; font-size: 18px; margin-bottom: 15px; color: var(--primary);");
        controlsDiv.appendChild(header);
     ...
     */
    async function cascadeDeleteRound(rIdx) {
        ...
        // Auto-save structural change
        if (typeof KSSS_UI_HOOKS !== 'undefined' && KSSS_UI_HOOKS.saveToGitHub) {
            await KSSS_UI_HOOKS.saveToGitHub();
        } else {
            await saveToGitHub();
        }
    }
    ...
    async function saveToGitHub() {
        ...
        if (!await verifyIntegrity()) {
            // Redundant alert removed as verifyIntegrity handles logout/alert
            return;
        }
        ...
        try {
            ...
            if (e.status === 409) {
                ...
                // Break infinite conflict loops by forcing a fresh load from GitHub
                loadMatches(true);
            } else {
                showStatus(`Error: ${e.message}`, "#ef4444");
            }
    ...
    function showBestLoserCreator(rIdx) {
        ...
        // Create backdrop (sibling, not parent)
        const backdrop = document.createElement("div");
     ...
     */
    async function createBestLoserMatch(rIdx) {
        ...
        // Auto-save to prevent match from disappearing on refresh
        if (typeof KSSS_UI_HOOKS !== 'undefined' && KSSS_UI_HOOKS.saveToGitHub) {
            await KSSS_UI_HOOKS.saveToGitHub();
        } else {
            await saveToGitHub();
     ...
     */
    async function finalizePairing(numMatches) {
        const matches = [];
    ...
    (function () {
        function secureHook(fn) {
            ...
            return async function (...args) {
                // New: Layer 3 Integrity Reinforcement
    ...
    // ============================================
    // FROZEN PUBLIC API SURFACE (Rule 2)
    // ============================================

    /**
     * KSSS_UI_HOOKS - The ONLY exported interface to window
     * All functions wrapped with authentication and initialization guards
     */
    const KSSS_UI_HOOKS = Object.freeze({
        // Authentication hooks
        login: function () { return AdminSecurity.login(); },
        logout: function () { return AdminSecurity.logout(); },

        // Core Data Operations
        loadMatches: function () { return loadMatches(...arguments); },
        saveToGitHub: function () { return saveToGitHub(...arguments); },

        // Team Swap System
        confirmTeamSwap: function () { return confirmTeamSwap(...arguments); },
        cancelTeamSwap: function () { return cancelTeamSwap(...arguments); },
        unlockTeam: function () { return unlockTeam(...arguments); },
        relockTeam: function () { return relockTeam(...arguments); },

        // History Management
        undoChange: function () { return undoChange(...arguments); },
        redoChange: function () { return redoChange(...arguments); },

        // Export Functions
        exportToCSV: function () { return exportToCSV(...arguments); },
        exportToPDF: function () { return exportToPDF(...arguments); },

        // Filtering & Pagination
        filterMatches: function () { return window.filterMatches ? window.filterMatches(...arguments) : console.warn('filterMatches not defined'); },
        clearFilters: function () { return window.clearFilters ? window.clearFilters(...arguments) : console.warn('clearFilters not defined'); },
        changePage: function () { return changePage(...arguments); },

        // Team Switch Mode
        activateTeamSwitchMode: function () { return activateTeamSwitchMode(...arguments); },
        exitTeamSwitchMode: function () { return exitTeamSwitchMode(...arguments); },

        // Round Management
        cascadeDeleteRound: function () { return window.cascadeDeleteRound ? window.cascadeDeleteRound(...arguments) : console.warn('cascadeDeleteRound not defined'); },
        endTournament: function () { return endTournament(...arguments); },
        showBestLoserCreator: function () { return showBestLoserCreator(...arguments); },
        showRoundGenerator: function () { return showRoundGenerator(...arguments); },
        createBestLoserMatch: function () { return createBestLoserMatch(...arguments); },
        startPairing: function () { return startPairing(...arguments); },
        finalizePairing: function () { return finalizePairing(...arguments); },

        // Modal Management
        closeBestLoserModal: function () { return window.closeBestLoserModal ? window.closeBestLoserModal(...arguments) : null; },
        closeRoundGenModal: function () { return window.closeRoundGenModal ? window.closeRoundGenModal(...arguments) : null; },
        closeStructuralLogModal: function () { return window.closeStructuralLogModal ? window.closeStructuralLogModal(...arguments) : null; }
    });

    // ============================================
    // IMMUTABLE GLOBAL EXPORT (Rules 1, 2, 20)
    // ============================================

    // Attach KSSS_UI_HOOKS as the ONLY global export
    Object.defineProperty(window, 'KSSS_UI_HOOKS', {
        value: KSSS_UI_HOOKS,
        writable: false,
        configurable: false,
        enumerable: true
    ...
    // LEGACY COMPATIBILITY: Direct window.login and window.logout for HTML
    // These call through to KSSS_UI_HOOKS to maintain single code path
    Object.defineProperty(window, 'login', {
        value: KSSS_UI_HOOKS.login,
        writable: false,
        configurable: false,
        enumerable: true
    });
    ...
    if (CONFIG?.debug) {
        console.log('🛡️ Admin.js Security Hardened v2.2.4');
        console.log('✅ IIFE Wrapper Active');
        console.log('✅ Global Exports:', Object.keys(window).filter(k => k.includes('KSSS') || k === 'login' || k === 'logout'));
    }
